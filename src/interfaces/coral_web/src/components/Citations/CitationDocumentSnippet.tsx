...
> = ({ toolId, document, keyword, onToggle }) => {
  ...
  return (
    <div className="flex flex-col">
      ...
      <button
        className="self-end p-0 text-coral-900 transition-colors ease-in-out hover:text-coral-700"
...
const Snippet: React.FC<{
  ...
  return (
    <Text className={cn('content text-coral-900', lineLimitClass)}>
...
const ConsoleOutput: React.FC<{ type: string; message: string }> = ({ type, message }) => {
  return (
    <>
      <Text as="span" styleAs="code-sm" className="text-coral-900">
        {type}
      </Text>
...
const CodeExecutionTime: React.FC<{ runtime: string }> = ({ runtime }) => {
  return (
    <Text as="span" styleAs="code-sm" className="mt-2 text-coral-900">
