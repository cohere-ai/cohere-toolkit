...
export const DragDropFileInput: React.FC<DragDropFileInputProps> = ({
  ...
  return (
    <div
      className={cn(
        'relative flex h-28 w-full flex-col items-center justify-center rounded-md border border-mushroom-200 px-3 py-6',
        'transition duration-200',
        'border-dashed bg-mushroom-50',
        {
          'border-solid bg-mushroom-200': dragActive,
        },
        className
      )}
    >
      {dragActive ? (
        <Text className="max-w-[170px] text-center text-mushroom-800">{dragActiveLabel}</Text>
      ) : (
        <>
          <Text className="max-w-[210px] text-center text-mushroom-800">{label}</Text>
          <Text className="text-center text-mushroom-700" styleAs="caption">
            {subLabel}
          </Text>
        </>
      )}
