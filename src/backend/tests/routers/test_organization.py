...
from backend.schemas.organization import CreateOrganization, UpdateOrganization, DeleteOrganization, Organization
...
def test_create_organization_with_existing_name(session_client: TestClient, session: Session) -> None:
    ...
    assert response.json() == {"detail": "Organization with name: test organization already exists."}
...
def test_update_organization(session_client: TestClient, session: Session) -> None:
    ...
    response = session_client.put(f"/v1/organizations/{organization.id}", json=new_organization.dict())
    assert response.status_code == 200
    assert response.json()["name"] == new_organization.name


def test_update_not_existing_organization(session_client: TestClient, session: Session) -> None:
    new_organization = UpdateOrganization(name="new organization")
...
def test_list_organizations(session_client: TestClient, session: Session) -> None:
    ...
    for i in range(5):
        organization = get_factory("Organization", session).create(name=f"test organization {i}")
