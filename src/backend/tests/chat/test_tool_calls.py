...
def test_async_call_tools_success() -> None:
    ...
    with patch.dict(AVAILABLE_TOOLS, MOCKED_TOOLS) as mock:
        results = asyncio.run(async_call_tools(chat_history, MockCohereDeployment(), ctx))
        assert results == [
            {
...
def test_async_call_tools_failure() -> None:
    ...
    with patch.dict(AVAILABLE_TOOLS, MOCKED_TOOLS) as mock:
        ...
        assert str(excinfo.value.detail) == "Error while calling tool toolkit_calculator: Calculator failed"
...
def test_async_call_tools_timeout() -> None:
    ...
    with patch.dict(AVAILABLE_TOOLS, MOCKED_TOOLS) as mock:
            ...
            assert str(excinfo.value.status_code) == '500'
            assert str(excinfo.value.detail) == "Timeout while calling tools with timeout: 1"
...
def test_async_call_tools_failure_and_success() -> None:
    ...
    with patch.dict(AVAILABLE_TOOLS, MOCKED_TOOLS) as mock:
        with pytest.raises(HTTPException) as excinfo:
            asyncio.run(async_call_tools(chat_history, MockCohereDeployment(), ctx))
        assert str(excinfo.value.status_code) == '500'
        assert str(excinfo.value.detail) == "Error while calling tool web_scrape: Web scrape failed"