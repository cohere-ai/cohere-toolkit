...
class GoogleDrive(BaseTool):
    ...
    async def call(self, parameters: dict, **kwargs: Any) -> List[Dict[str, Any]]:
        ...
        # index_name = "{}_{}".format(agent_id if agent_id is not None else user_id, self.NAME)
        ...
        # tool_auth = tool_auth_crud.get_tool_auth(db=session, tool_id=GOOGLE_DRIVE_TOOL_ID, user_id=user_id)
        ...
        # folder_ids = []
        # if agent_id:
        #     agent_metadata = get_all_agent_tool_metadata_by_agent_id(db=session, agent_id=agent_id)
        ...
        # if file_ids:
        #     files = get_file.perform(file_ids=file_ids, creds=creds)
        # else:
        ...
        #         conditions.append(
        #             "(" + " or ".join(["'{}' in parents".format(folder_id) for folder_id in folder_ids]) + ")"
        #         )
        ...
        #         self._handle_tool_specific_errors(error=e, **{"session": session, "user_id": user_id})
        ...
        # # native files
        # native_files = [x for x in processed_files if x["mimeType"] in NATIVE_SEARCH_MIME_TYPES]
        # id_to_urls = extract_links(native_files)
        # if id_to_urls:
        #     id_to_texts = await download.async_perform(id_to_urls, creds.token)
        ...
        # # non-native files
        # non_native_files = [x for x in processed_files if x["mimeType"] in NON_NATIVE_SEARCH_MIME_TYPES]
        # non_native_results = await non_native_files_perform(service=service, compass=compass, files=non_native_files)
        # id_to_texts = {
        #     **id_to_texts,
        #     **non_native_results,
        # }
