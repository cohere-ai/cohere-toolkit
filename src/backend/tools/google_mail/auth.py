...
from backend.tools.google_mail.tool import GoogleMail
...
class GoogleMailAuth(BaseToolAuthentication, ToolAuthenticationCacheMixin):
    TOOL_ID = GoogleMail.NAME
    ...
    def try_refresh_token(
        ...
        if response.status_code != 200:
            logger.error(
                event=f"[Google Mail] Error refreshing token: {response_body}"
            )
            return False
