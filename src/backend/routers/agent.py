...
async def create_agent(
    config: ConfigDep, agent: CreateAgent, request: Request
) -> AgentPublic:
    """
    Create an agent.
    Args:
...
async def list_agents(
    *, offset: int = 0, limit: int = 100, config: ConfigDep, request: Request
) -> list[AgentPublic]:
    """
    List all agents.
...
async def get_agent_by_id(agent_id: str, config: ConfigDep, request: Request) -> Agent:
    """
    Args:
...
async def update_agent(
    ...
    with config.db.session() as session, session.begin():
        ...
        if new_agent.tools_metadata is not None:
            ...
            agent = await handle_tool_metadata_update(
                agent, new_agent, session, request
            )
...
async def handle_tool_metadata_update(
    agent: Agent, new_agent: Agent, config: ConfigDep, request: Request
) -> Agent:
    with config.db.session() as session, session.begin():
...
async def delete_agent(
    agent_id: str, config: ConfigDep, request: Request
) -> DeleteAgent:
    """
...
def create_agent_tool_metadata(
    ...
    with config.db.session() as session, session.begin():
        ...
        try:
            ...
            add_agent_tool_metadata_to_request_state(
                request, created_agent_tool_metadata
            )
        except Exception as e:
...
async def delete_agent_tool_metadata(
    agent_id: str, agent_tool_metadata_id: str, config: ConfigDep, request: Request
) -> DeleteAgentToolMetadata:
    """
