...
async def login(request: Request, login: Login, config: ConfigDep):
    """
    ...
    with config.db.session() as session, session.begin():
        ...
        if not is_enabled_authentication_strategy(strategy_name):
            raise HTTPException(
                status_code=422,
                detail=f"Invalid Authentication strategy: {strategy_name}.",
            )
...
async def authorize(
    strategy: str, request: Request, config: ConfigDep, code: str = None
):
    """
...
async def login(request: Request, config: ConfigDep):
    """
    Logs user in, performing basic email/password auth.
    Verifies their credentials, retrieves the user and returns a JWT token.
