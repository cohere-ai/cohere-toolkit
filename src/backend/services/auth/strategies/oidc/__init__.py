...
from backend.services.auth.strategies.oidc.env import OIDCEnvironment
...
class OpenIDConnect(BaseOAuthStrategy):
    ...
    def __init__(self, env: OIDCEnvironment):
        try:
            self._env = env
            self.REDIRECT_URI = (
                f"{self._env.frontend_hostname}/auth/{self.NAME.lower()}"
            )
            self.WELL_KNOWN_ENDPOINT = self._env.oidc_well_known_endpoint
    ...
    def get_client_id(self):
        return self._env.oidc_client_id
