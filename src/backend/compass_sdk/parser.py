...
class CompassParserClient:
    ...
    def process_file(
        ...
        *,
        ...
        if doc.errors:
            logger.error(event=f"Error opening document: {doc.errors}")
            return []
        if len(doc.filebytes) > DEFAULT_MAX_ACCEPTED_FILE_SIZE_BYTES:
            logger.error(
                event=f"File too large, supported file size is {DEFAULT_MAX_ACCEPTED_FILE_SIZE_BYTES / 1000_000} "
        ...
        if res.ok:
            ...
            docs = []
            logger.error(event=f"Error processing file: {res.text}")
    ...
    def batch_upload(self, *, zip_file_path: str) -> str:
        ...
        if res.ok:
            ...
            logger.error(event=f"Error uploading file: {res.text}")
    ...
    def batch_status(self, uuid: str) -> str:
        ...
        if res.ok:
            ...
            logger.error(event=f"Error getting batch status: {res.text}")
