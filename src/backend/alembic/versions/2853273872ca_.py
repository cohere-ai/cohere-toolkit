...
def upgrade() -> None:
    ...
    op.create_index('citation_message_id_user_id', 'citations', ['message_id', 'user_id'], unique=False)
    ...
    op.create_index('document_conversation_id', 'documents', ['conversation_id'], unique=False)
    op.create_index('document_conversation_id_user_id', 'documents', ['conversation_id', 'user_id'], unique=False)
...
def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('messages', sa.Column('type', sa.VARCHAR(length=4), autoincrement=False, nullable=False))
